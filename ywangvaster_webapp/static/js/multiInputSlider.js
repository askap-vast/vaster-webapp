...
,
    ...
    createSlider(parent, sliderLabel = "", numSliders = 2, minSliderValue = 0.0, maxSliderValue = 1.0, initialValues = null ) {
      ...
      if (initialValues) {
        ...
        if ( !(parseInt(numSliders) == 1)) {
          if (initialValues.length == parseInt(numSliders) ) {
            values = initialValues.map( (v, i) => v ? ((parseFloat(v)-parseFloat(minSliderValue))/minMaxDiff) : i / (numSliders - 1) )
          } else {
